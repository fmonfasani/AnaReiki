-- ============================================================
-- MIGRATION 045: Fix timezone en get_available_slots_v2
-- Las TIME (start_time, end_time) se ingresan en hora Argentina
-- (UTC-3), pero la funcion las interpretaba como UTC.
-- Se usa AT TIME ZONE 'America/Argentina/Buenos_Aires' para
-- que PostgreSQL convierta correctamente a UTC timestamptz.
-- ============================================================
  ...
  LOOP
    ...
    WHILE v_offset + v_rules.duration_minutes <= v_window_minutes LOOP
      IF v_rules.promotion_id IS NOT NULL THEN
        RETURN QUERY
        SELECT
          (p_date + v_rules.start_time + (v_offset || ' minutes')::interval) AT TIME ZONE v_tz,
          (p_date + v_rules.start_time + ((v_offset + v_rules.duration_minutes) || ' minutes')::interval) AT TIME ZONE v_tz,
          v_rules.id,
          CASE
          ...
          (
            ...
            WHERE a.start_time = ((p_date + v_rules.start_time + (v_offset || ' minutes')::interval) AT TIME ZONE v_tz)
