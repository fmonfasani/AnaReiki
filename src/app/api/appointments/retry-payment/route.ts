...
export async function POST(request: Request) {
  try {
    ...
    const { data: appointment, error: apptError } = await svc
      .from("appointments")
      .select("id, status, payment_status, price_cents, deposit_cents, balance_cents, service_id, client_id")
      .eq("id", appointmentId)
    ...
    const mpAmount = (appointment.deposit_cents || appointment.price_cents || 0) / 100;
