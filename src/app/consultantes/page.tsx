...
const MILESTONE_LABELS: Record<number, { label: string; emoji: string }> = {
  7: { label: "7 días", emoji: "🌱" },
  30: { label: "30 días", emoji: "🌿" },
  60: { label: "60 días", emoji: "🌳" },
  90: { label: "90 días", emoji: "🌸" },
  180: { label: "180 días", emoji: "🪷" },
...
function calculateStreak(reflections?: { created_at: string }[]) {
  ...
  for (const entry of sorted) {
    ...
    if (!lastDate) { streak = 1; lastDate = day; continue; }
    const diff = differenceInCalendarDays(lastDate, day);
    if (diff === 0) continue;
    if (diff === 1) { streak += 1; lastDate = day; continue; }
    break;
  }
  return streak;
}
...
export default async function MembersDashboard() {
  ...
  const [nextAppointment, sessionNotes, reflections, recentContent, availability, oracleData, streakData, sessionHistory, profileResult] =
    await Promise.all([
      ...
      supabase.from("daily_reflections").select("*").eq("user_id", user.id).order("created_at", { ascending: false }).limit(14),
      supabase.from("content").select("*").order("published_at", { ascending: false }).limit(3),
      supabase.from("availability_rules").select("*").eq("is_active", true).order("day_of_week").order("start_time"),
  ...
  const dbOracle = oracleData.data || [];
  const oracleQuote = dbOracle.length > 0
  ...
  const nextMilestone = [7, 30, 60, 90, 180, 365].find((m) => !achievedMilestones.includes(m));
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="bg-gradient-to-r from-purple-700 to-pink-500 rounded-3xl p-8 text-white shadow-lg relative overflow-hidden">
        <div className="relative z-10 max-w-3xl">
          <div className="flex items-center gap-3 mb-4 flex-wrap">
            ...
            {achievedMilestones.map((m: number) => {
              const ms = MILESTONE_LABELS[m];
              return ms ? (
                <span key={m} className="px-3 py-1 bg-yellow-300/20 backdrop-blur-md rounded-full text-xs font-bold border border-yellow-300/30 flex items-center gap-1">
                  {ms.emoji} {ms.label}
                </span>
              ) : null;
            })}
        ...
        <div className="absolute right-0 bottom-0 opacity-20 transform translate-x-10 translate-y-10">
          <svg width="300" height="300" viewBox="0 0 200 200">
            <path fill="currentColor" d="M45.7,-76.3C58.9,-69.3,69.1,-55.6,76.5,-40.9C83.9,-26.2,88.5,-10.6,86.6,4.1C84.7,18.8,76.3,32.6,66.3,44.4C56.3,56.2,44.7,66,31.8,70.9C18.9,75.8,4.7,75.9,-8.8,73.8C-22.3,71.7,-35.1,67.4,-46.8,59.3C-58.5,51.2,-69.1,39.3,-75.6,25.3C-82.1,11.3,-84.5,-4.8,-80.4,-19.4C-76.3,-34,-65.7,-47.1,-53.4,-54.6C-41.1,-62.1,-27.1,-64,-13.6,-66.9C-0.1,-69.8,13.4,-73.7,32.5,-83.3L45.7,-76.3Z" transform="translate(100 100)" />
          </svg>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            ...
            {nextMilestone && (
              <p className="text-xs text-amber-600 mt-2 flex items-center gap-1">
                ...
                Próximo hito: {MILESTONE_LABELS[nextMilestone]?.emoji} {MILESTONE_LABELS[nextMilestone]?.label || `${nextMilestone} días`}
              </p>
            )}
          </div>
          <Link href="/consultantes/evolucion" className="mt-6 w-full py-2 px-4 bg-gray-50 hover:bg-gray-100 text-gray-700 font-medium rounded-xl text-center text-sm transition-colors border border-gray-200">
            Ver Bitácora
          </Link>
        </div>
            ...
            {nextAppointment.data ? (
              <div>
                <p className="text-2xl font-bold text-gray-800">
                  {new Date(nextAppointment.data.start_time).toLocaleDateString("es-ES", { weekday: "long", day: "numeric" })}
                </p>
                <p className="text-pink-600 font-medium text-lg">
                  {new Date(nextAppointment.data.start_time).toLocaleTimeString("es-ES", { hour: "2-digit", minute: "2-digit" })} hs
                </p>
                <span className="inline-block mt-2 px-3 py-1 bg-green-100 text-green-700 text-xs rounded-full font-bold uppercase tracking-wide">Confirmada</span>
              </div>
            ) : (
              <p className="text-gray-500 text-sm">No tienes sesiones agendadas próximamente.</p>
            )}
          </div>
          <Link href="/consultantes/mis-citas" className="mt-6 w-full py-2 px-4 bg-gray-50 hover:bg-gray-100 text-gray-700 font-medium rounded-xl text-center text-sm transition-colors border border-gray-200">
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500 mb-4">Próximos turnos disponibles:</p>
            <div className="space-y-2">
              {nextSlots.length > 0 ? nextSlots.map((slot, i) => (
                <div key={i} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg text-xs">
                  <span className="font-medium text-gray-700">{DAY_LABELS[slot.day_of_week]}</span>
                  <span className="text-teal-600 font-bold">{slot.start_time.slice(0, 5)} hs</span>
                </div>
              )) : <p className="text-[10px] text-gray-400 italic">No hay horarios definidos esta semana.</p>}
            </div>
          </div>
          <Link href="/consultantes/reservar" className="mt-4 w-full py-2 px-4 bg-teal-50 hover:bg-teal-100 text-teal-700 font-bold rounded-xl text-center text-xs transition-colors border border-teal-100">
            Ver Agenda Completa
          </Link>
        </div>
      </div>

      {sessionHistory && sessionHistory.data && sessionHistory.data.length > 0 && (
        <section className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link href="/consultantes/evolucion" className="text-pink-600 font-bold text-sm hover:underline flex items-center gap-1">
              Ver todas <span className="material-symbols-outlined text-[16px]">arrow_forward</span>
            </Link>
          </div>
          <div className="space-y-3">
            {(sessionHistory.data as Array<{ id: string; title: string; created_at: string; mood_after?: number; tags?: string[] }>).slice(0, 3).map((entry) => (
              <div key={entry.id} className="flex items-start gap-3 p-4 bg-gray-50 rounded-xl">
                <div className="w-8 h-8 rounded-full bg-pink-100 flex items-center justify-center shrink-0 text-sm">
                  {entry.mood_after ? ["😫", "😕", "😐", "🙂", "🤩"][entry.mood_after - 1] : "📝"}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  {entry.tags && entry.tags.length > 0 && (
                    <div className="flex gap-1 mt-1 flex-wrap">
                      {entry.tags.map((t, i) => <span key={i} className="px-2 py-0.5 bg-pink-50 text-pink-600 rounded-full text-[10px]">{t}</span>)}
                    </div>
                  )}
                </div>
              </div>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          onClick={() => {
            const section = document.getElementById('practice-cards');
            if (section) {
      ...
      <div id="practice-cards" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
      ...
      <section className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm">
        ...
        {recentContent.data && recentContent.data.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {recentContent.data.map((item: { id: string; type: string; thumbnail_url?: string; title: string; description?: string }) => (
              <Link key={item.id} href={item.type === "video" ? `/consultantes/clases/${item.id}` : `/consultantes/podcast/${item.id}`} className="group flex flex-col">
                <div className="aspect-video bg-gray-100 rounded-2xl mb-3 overflow-hidden border border-gray-50 relative">
                  {item.thumbnail_url && <img src={item.thumbnail_url} alt={item.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />}
                  <div className="absolute top-2 right-2 px-2 py-1 bg-black/50 backdrop-blur-sm rounded-lg text-[10px] text-white font-bold uppercase">{item.type}</div>
                </div>
                <h4 className="font-bold text-gray-900 group-hover:text-pink-600 transition-colors line-clamp-1">{item.title}</h4>
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-gray-400 max-w-sm">Pronto aparecerán aquí tus últimas clases y episodios del podcast.</p>
          </div>
        )}
      </section>
    </div>
